...
python3 $SOURCE_DIR/run_clingo.py $CLINGO_FILE $PROJECT_NAME

#Parse the clingo output and store the produced Pandas Dataframes
python3 $SOURCE_DIR/load_worlds.py $PROJECT_NAME

#Export the dataframes to sql and csv formats
python3 $SOURCE_DIR/export.py -p $PROJECT_NAME -csv -sql

#Need to run this to use the visualisation script. Calls a dummy distance function which does no computation.
python3 $SOURCE_DIR/dist_calc.py -p $PROJECT_NAME -custom_dist_func dummy_dist_func -calc_dist_matrix
...
python3 $SOURCE_DIR/visualize.py -p $PROJECT_NAME -custom_visualization_func euler_visualization_nxpd

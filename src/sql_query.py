...
def freq_sqlite(rl_id=0, col_names=[], values=[], pws_to_consider=[j for j in range(1, expected_pws + 1)],
                do_print=True):
    global relations
    global expected_pws
    global dfs
...
def redundant_column_sqlite(rl_id=0, col_names=[], pws_to_consider=[j for j in range(1, expected_pws + 1)],
                            do_print=True):
    global relations
    global expected_pws
    global dfs
